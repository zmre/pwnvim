...
vim.api.nvim_set_keymap("", "<c-p>", ":silent Telescope find_files<CR>", options)
vim.api.nvim_set_keymap("!", "<c-p>", "<ESC>:silent Telescope find_files<CR>", options)

-- Make F4 toggle showing invisible characters
vim.api.nvim_set_keymap("", "_z", ":set list<CR>:map #4 _Z<CR>", { silent = true })
vim.api.nvim_set_keymap("", "_Z", ":set nolist<CR>:map #4 _z<CR>", { silent = true })
...
vim.api.nvim_set_keymap("!", "<F12>", "<C-o>:syntax sync fromstart<CR>", options)

-- Have ctrl-l continue to do what it did, but also temp clear search match highlighting
vim.api.nvim_set_keymap("", "<C-l>", ":<C-u>nohlsearch<CR><C-l>", { silent = true })
...
local leader_mappings = {
  ...
  i = {
    ...
    ["2"] = {
      "<cmd>lua require('pwnvim.options').twospaceindent()<CR>",
      "Two Space",
    },
    ["4"] = {
  ...
  t = {
    ...
    d = {
      function()
        require("pwnvim.tasks").completeTaskDirect()
      end,
      "Done",
    },
    c = {
      function()
        require("pwnvim.tasks").createTaskDirect()
      end,
      "Create",
    },
    s = {
      function()
        require("pwnvim.tasks").scheduleTaskPrompt()
      end,
      "Schedule",
    },
    t = {
      function()
        require("pwnvim.tasks").scheduleTaskTodayDirect()
      end,
      "Today",
    },
  },
...
vim.api.nvim_set_keymap("", "<leader>fd", ":silent Telescope lsp_document_symbols<CR>", options)
...
vim.api.nvim_set_keymap("", "<D-k>", "gk", options)
...
vim.api.nvim_set_keymap("", "gx", ':silent !open "<c-r><c-f>" || xdg-open "<c-r><c-f>"<cr>', options)
vim.api.nvim_set_keymap("v", "gx", '"0y:silent !open "<c-r>0" || xdg-open "<c-r>0"<cr>gv', options)
...
vim.api.nvim_set_keymap("", "<Space>", [[@=(foldlevel('.')?'za':"\<Space>")<CR>]], options)
...
vim.api.nvim_set_keymap("!", "<c-h>", "<Esc>:TmuxNavigateLeft<cr>", { silent = true })
vim.api.nvim_set_keymap("!", "<c-j>", "<Esc>:TmuxNavigateDown<cr>", { silent = true })
vim.api.nvim_set_keymap("!", "<c-k>", "<Esc>:TmuxNavigateUp<cr>", { silent = true })
vim.api.nvim_set_keymap("!", "<c-l>", "<Esc>:TmuxNavigateRight<cr>", { silent = true })
